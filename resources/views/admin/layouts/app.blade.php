...
<head>
    ...
    <title>@yield('title', 'Admin') — {{ \App\Models\Setting::getValue('app_name', 'OpenMosque') }}</title>
...
<body class="bg-gray-50 text-gray-800 custom-scrollbar relative" x-data="{ sidebarOpen: false }">
    {{-- Subtle Islamic Pattern (Optional, kept very light for branding) --}}
    ...
    <div class="flex min-h-screen relative z-10">
        ...
        {{-- Sidebar --}}
        <aside
            class="admin-sidebar fixed md:sticky top-0 left-0 h-screen w-[260px] flex flex-col z-50 transition-transform duration-300 md:translate-x-0"
        ...
        >
            ...
            <div class="p-5 border-b border-gray-100">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3">
                    ...
                    @endif
                    <div>
                        <h1 class="text-lg font-bold font-heading text-emerald-800">{{ \App\Models\Setting::getValue('app_name', 'OpenMosque') }}</h1>
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
                ...
                <a href="{{ route('admin.dashboard') }}"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
                ...
                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.announcements.*') ? 'active' : '' }}">
                    <i data-lucide="megaphone" class="w-[18px] h-[18px]"></i>
                    <span>{{ __('Announcements') }}</span>
                </a>

                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.events.*') ? 'active' : '' }}">
                    <i data-lucide="calendar-days" class="w-[18px] h-[18px]"></i>
                    <span>{{ __('Events') }}</span>
                </a>

                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.pages.*') ? 'active' : '' }}">
                    <i data-lucide="file-text" class="w-[18px] h-[18px]"></i>
                    <span>{{ __('Pages') }}</span>
                </a>

                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.media.*') ? 'active' : '' }}">
                ...
                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.prayer-times.*') ? 'active' : '' }}">
                    <i data-lucide="clock" class="w-[18px] h-[18px]"></i>
                    <span>{{ __('Prayer Times') }}</span>
                </a>

                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.donations.*') ? 'active' : '' }}">
                    <i data-lucide="heart-handshake" class="w-[18px] h-[18px]"></i>
                    <span>{{ __('Donations') }}</span>
                </a>

                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.services.*') ? 'active' : '' }}">
                    <i data-lucide="hand-helping" class="w-[18px] h-[18px]"></i>
                    <span>{{ __('Services') }}</span>
                </a>

                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.staff.*') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.settings.index') }}"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.settings.*') ? 'active' : '' }}">
                    <i data-lucide="settings" class="w-[18px] h-[18px]"></i>
                    <span>{{ __('Settings') }}</span>
                </a>

                <a href="#"
                   class="nav-link flex items-center gap-3 px-3 py-2.5 text-sm {{ request()->routeIs('admin.users.*') ? 'active' : '' }}">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            {{-- Top Header --}}
            <header class="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 md:px-6 py-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        {{-- Mobile Menu Toggle --}}
                        <button type="button" @click.prevent="sidebarOpen = !sidebarOpen" class="md:hidden p-2 text-gray-600 hover:text-gray-900 rounded-lg hover:bg-gray-100">
                        ...
                        <div class="hidden md:flex items-center gap-2 text-sm text-gray-500">
                            ...
                            <span class="text-gray-800 font-medium">@yield('breadcrumb', __('Dashboard'))</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div x-data="{ userOpen: false }" class="relative">
                            <button @click="userOpen = !userOpen" class="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-white/5 transition-colors">
                                ...
                                <span class="hidden md:block text-sm text-gray-700 font-medium">{{ auth()->user()->name ?? 'Admin' }}</span>
                            ...
                            <div x-show="userOpen" @click.outside="userOpen = false"
                                ...
                                <a href="#" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100">
